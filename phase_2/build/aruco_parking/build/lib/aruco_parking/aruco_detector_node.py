...
class ArucoDetectorNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('marker_size_m', 0.067) #it has to be 0.067
        self.declare_parameter('dict_id',       16) # it has to be 16
        ...
        # Temporal persistence — only declare "not detected" after this many
        # consecutive missed frames. Prevents flickering detected/lost/detected
        # that causes the robot to stop-start during approach.
        self._no_detect_frames  = 0
        self._LOST_THRESHOLD    = 5   # frames before declaring lost
        ...
        # Navigator publishes which ID it currently wants — prefer that one
        # when multiple markers are visible. -1 means no preference.
        self._target_id = -1
        self.create_subscription(
            Int32, '/aruco/target_id',
            lambda msg: setattr(self, '_target_id', msg.data), 10)
    ...
    def _init_aruco(self, dict_id):
        try:
            ...
            # Markers are flat on the floor viewed from above at an angle,
            # so they appear as trapezoids not squares. These settings make
            # the detector more tolerant of perspective-distorted shapes and
            # sample the marker pattern more robustly.
            self._aruco_params.polygonalApproxAccuracyRate          = 0.08  # default 0.05
            self._aruco_params.minCornerDistanceRate                 = 0.02  # default 0.05
            self._aruco_params.perspectiveRemovePixelPerCell         = 8     # default 4
            self._aruco_params.perspectiveRemoveIgnoredMarginPerCell = 0.2   # default 0.13
            ...
            self._aruco_params.polygonalApproxAccuracyRate          = 0.08
            self._aruco_params.minCornerDistanceRate                 = 0.02
            self._aruco_params.perspectiveRemovePixelPerCell         = 8
            self._aruco_params.perspectiveRemoveIgnoredMarginPerCell = 0.2
            self._detector     = None
            self._new_api      = False
    ...
    def image_callback(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            # Reset the no-detect counter — marker is visible
            self._no_detect_frames = 0
            self._last_detected    = True

            # If the navigator has requested a specific ID, prefer that one.
            # Otherwise fall back to the first detected marker (ids[0]).
            chosen_idx = 0
            if self._target_id >= 0:
                for i, mid in enumerate(ids):
                    if int(mid[0]) == self._target_id:
                        chosen_idx = i
                        break

            tag_id      = int(ids[chosen_idx][0])
            tag_corners = corners[chosen_idx][0]   # (4, 2)
            ...
            # avoids flickering from a single noisy/occluded frame.
            if self._no_detect_frames >= self._LOST_THRESHOLD:
                self._last_detected = False
